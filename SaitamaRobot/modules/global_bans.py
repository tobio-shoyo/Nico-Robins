...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is your daddy! And mine also! I can't act against our own.",
        )
        return
    ...
    if int(user_id) in DEMONS:
        message.reply_text(
            "OOOH someone's trying to gban a teacher! *grabs popcorn*",
        )
        return

    if int(user_id) in TIGERS:
        message.reply_text("That's a Sen-pai! They cannot be banned!")
        return

    if int(user_id) in WOLVES:
        message.reply_text("That's a kohai! They cannot be banned!")
        return
